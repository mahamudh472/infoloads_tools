...
export default function SearchOverlay({
  ...
  onSelectTool: propsOnSelectTool,
  initialCategory: propsInitialCategory,
}: SearchOverlayProps) {
  const router = useRouter();
  const ui = useUI();

  const isOpen = propsIsOpen !== undefined ? propsIsOpen : ui.searchOpen;
  const onClose = propsOnClose || ui.closeSearch;
  const initialCategory = propsInitialCategory || ui.searchCategory || "all";
  ...
  // Filter tools
  const rawResults = searchTools(searchQuery, selectedCategoryTab === "ai" ? "all" : selectedCategoryTab);
  const filteredTools = selectedCategoryTab === "ai"
    ? rawResults.filter((t) => t.popular)
    : rawResults;

  const handleSelect = (toolId: string) => {
    if (propsOnSelectTool) {
      propsOnSelectTool(toolId);
    } else {
      const tool = rawResults.find((t) => t.id === toolId || t.slug === toolId);
      if (tool) {
